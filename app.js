...
class ChatUI {
  ...
  showEmptyState(){ if(this.emptyState) this.emptyState.classList.remove('hidden'); }
  hideEmptyState(){ if(this.emptyState) this.emptyState.classList.add('hidden'); }
...
class ChatApp {
  ...
  async onPolish(){
    const txt = (this.ui.messageInput && this.ui.messageInput.value || '').trim();
    if(!txt){ if(this.ui.messageInput) this.ui.messageInput.focus(); return; }
    const tone = (this.ui.toneSelect && this.ui.toneSelect.value) || 'professional';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(saveToneBtn && profileToneSelect){
    saveToneBtn.addEventListener('click', ()=>{
      ...
      if(saveToneMsg){
        ...
        setTimeout(()=>{ saveToneMsg.style.display = 'none'; }, 1500);
      }
    });
  }
  ...
  function showView(name){
    ...
    views.forEach(v=>{
      ...
      if(!el) return;
      if(v === `view-${name}`) el.classList.remove('hidden'); else el.classList.add('hidden');
    });
    // highlight nav
